...
协议栈相关
*/
const char *GateWayName = "ESP8266Demo";
char GateWaySerialNumber[32] = "8266";
...
static bool mqttc_event_callback(esp_mqtt_event_id_t event_id, esp_mqtt_event_handle_t event) //返回true表示消息已处理，无需其它处理
{
    ...
    case MQTT_EVENT_DATA:
    {
        ...
        SMGS_GateWay_Receive_MQTT_MSG(&gateway_context, event->topic, event->topic_len, (uint8_t *)event->data, event->data_len, 0, 0, buff, buffsize);
        free(buff);
